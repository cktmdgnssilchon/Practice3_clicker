...
public class UIScrollBar : MonoBehaviour
{
			...
			if (mDir != value)
			{
				...
				if (mBG != null)
				{
						...
						(mDir == Direction.Horizontal && scale.x < scale.y))
					{
						...
						if (mBG.gameObject.GetComponent<Collider>() != null) NGUITools.AddWidgetCollider(mBG.gameObject);
						if (mFG.gameObject.GetComponent<Collider>() != null) NGUITools.AddWidgetCollider(mFG.gameObject);
					}
				}
			}
		}
	...
	void Start ()
	{
		if (background != null && background.gameObject.GetComponent<Collider>() != null)
		{
		...
		if (foreground != null && foreground.gameObject.GetComponent<Collider>() != null)
		{
